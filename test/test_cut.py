...
class TestCut(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        logging.info(
            "应存在文件列表：" + str(TEST_VIDEO_FILE_SIMPLE) + "  扫描到文件列表：" + str(scan_file)
        )
    ...
    def test_srt_cut(self, file_name):
        args = TestArgs()
        args.inputs = [
            TEST_VIDEO_PATH + file_name,
            TEST_CONTENT_PATH + file_name.split(".")[0] + "_srt.srt",
        ]
        cut = Cutter(args)
        cut.run()
        self.assertTrue(
            os.path.exists(TEST_VIDEO_PATH + file_name.split(".")[0] + "_cut.mp4")
        )
    ...
    def test_md_cut(self, file_name):
        ...
        args.inputs = [
            TEST_VIDEO_PATH + file_name,
            TEST_CONTENT_PATH + file_name.split(".")[0] + ".srt",
