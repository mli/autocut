...
class TestTranscribe(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        logging.info("应存在文件列表：" + str(TEST_VIDEO_FILE) + str(TEST_VIDEO_FILE_LANG) + str(TEST_VIDEO_FILE_SIMPLE) +
                     "  扫描到文件列表：" + str(scan_file))
        for file in TEST_VIDEO_FILE:
            assert file in scan_file
        for file in TEST_VIDEO_FILE_LANG:
            assert file in scan_file
    ...
    def tearDownClass(cls):
        for file in os.listdir(TEST_VIDEO_PATH):
            if file.endswith("md") or file.endswith("srt"):
                os.remove(TEST_VIDEO_PATH+file)

    def tearDown(self):
        for file in TEST_VIDEO_FILE_SIMPLE:
            if os.path.exists(TEST_VIDEO_PATH+file.split('.')[0]+'.md'):
                os.remove(TEST_VIDEO_PATH+file.split('.')[0]+'.md')
            if os.path.exists(TEST_VIDEO_PATH + file.split('.')[0] + '.srt'):
                os.remove(TEST_VIDEO_PATH + file.split('.')[0] + '.srt')

    @parameterized.expand([param(file) for file in TEST_VIDEO_FILE])
    def test_default_transcribe(self, file_name):
        logging.info("检查默认参数生成字幕")
        args = TestArgs()
        args.inputs = [TEST_VIDEO_PATH+file_name]
        transcribe = Transcribe(args)
        transcribe.run()
        self.assertTrue(os.path.exists(TEST_VIDEO_PATH+file_name.split('.')[0]+'.md'))
    ...
    @parameterized.expand([param(file) for file in TEST_VIDEO_FILE_LANG])
    ...
    def test_force_transcribe(self, file_name):
        ...
        args.force = True
        args.inputs = [TEST_VIDEO_PATH+file_name]
        md0_lens = len(''.join(MD(TEST_VIDEO_PATH + file_name.split('.')[0] + '.md', args.encoding).lines))
        transcribe = Transcribe(args)
        transcribe.run()
        md1_lens = len(''.join(MD(TEST_VIDEO_PATH+file_name.split('.')[0]+'.md', args.encoding).lines))
    ...
    def test_encoding_transcribe(self, file_name):
        ...
        transcribe = Transcribe(args)
        transcribe.run()
        with open(os.path.exists(TEST_VIDEO_PATH+file_name.split('.')[0]+'.md'), encoding='gbk'):
